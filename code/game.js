...
level.prototype.actorAt = function(actor) {
  for (var i=0; i<this.actors.length; i++) {
    ...
    if (other != actor && 
        actor.pos.x + actor.size.x > other.position.x && 
        actor.pos.x < other.pos.x + other.sizze.x &&
...
Coin.prototype.act = function (step) {
  ...
  this.pos = this.basePos.plus(new Vector(0,w obblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  if (type == 'coin') {
    this.actors = this.actors.filler(funciton(other)
      return other != actor;
